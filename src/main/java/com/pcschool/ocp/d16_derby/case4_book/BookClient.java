...
public class BookClient {
    public static BookCRUD crud = new BookCRUD();
    ...
    public void add() {
        // 取得使用者輸入
        Scanner sc = new Scanner(System.in);
        System.out.print("請輸入書名(bename)");
        String bename = sc.next();
        System.out.print("請輸入價格(price)");
        int price = sc.nextInt();
        // 建立 book 物件, 並將使用者所輸入的資料注入
        Book book = new Book();
        book.setBname(bename);
        book.setPrice(price);
        // 進行資料庫新增
    ...
    public void update() {
        ...
        if (sc.next().equals("y")) {
            System.out.println("請輸入書名(bename):");
